...
aria2c -x10 "$a1" -o ttt/${name}.zip ; unzip ttt/${name}.zip ; wget https://kgithub.com/extdomains/raw.github.com/zhlhlf/make-Radio/main/payload-dumper-go ; chmod 777 payload-dumper-go ; aria2c -x10 "${flash_scripts}" -o shelll.7z ; 7z x shelll.7z ; mv ab_flash_scripts ${name} ; mkdir ${name}/images ; log=$(./payload-dumper-go -o limages payload.bin) ; for i in $image ; do mv limages/$i ${name}/images/ ; done ; mv limages/my_* ${name}/images/ || echo "没有my_*\n"; rm -r limages ; rm -r payload.bin && rm -r ttt ; aria2c -x10 "$fw" -o zzzz.zip ; unzip zzzz.zip -d ${name} ; rm -r ${name}/META-INF ; 7z a ${name}.7z ${name} ; rm -r 666 || echo "666" ; mkdir 666 ; mv ${name}.7z 666 ; wget https://raw.kgithub.com/zhlhlf/text/main/onedrive_mount.sh ; sh onedrive_mount.sh 0000 临时存放文件 ；
